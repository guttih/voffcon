...
function getUserDeviceMonitorList(){
	var url = SERVER+'/monitors/device-list';
		var request = $.get(url);
...
/*Creates an item for a list which shows which devices have monitors and how many monitors*/
function createDevicesMonitorList(id, name, description, monitorRecordCount, routeText){
...
'<div id="listItem'+ id +'" class="list-group-item clearfix">' +
	...
	strElm +='<button onclick="deviceMonitors(\''+id+'\');" class="btn btn-xs btn-success"> <span class="glyphicon glyphicon-play"></span>&nbsp;Monitors('+ monitorRecordCount +') </button>';
...
//opens the page for monitors for the the specified device.
...
function setDevicelistValues(deviceList){
	...
	for(var i = 0; i < deviceList.length; i++){
		...
		var str =  createDevicesMonitorList(id, name, description, recordCount, 'monitors');
